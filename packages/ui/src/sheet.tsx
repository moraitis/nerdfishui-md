...
const SheetPortal = ({
  ...
  <SheetPrimitive.Portal {...props}>
    <div className={cx(portalVariants({position}), className)}>{children}</div>
...
const sheetVariants = cva(
  'bg-primary shadow-outline fixed z-50 scale-100 gap-4 p-6 opacity-100 shadow-lg',
  {
    variants: {
      position: {
...
>(({position, size, className, children, ...props}, ref) => (
  <SheetPortal position={position}>
    ...
    <SheetPrimitive.Content
      ref={ref}
      className={cx(sheetVariants({position, size}), className)}
      {...props}
    >
      {children}
...
const Sheet = Object.assign(RawSheet, {
